...
- **It names or clearly matches one workflow above** — start it and follow it to the end:
  `node "${CLAUDE_PLUGIN_ROOT}/scripts/gate-workflow.mjs" run <id> --watch "<task text>"`, where the
  task text is everything the user wrote apart from the workflow name. If the workflow needs more
  than one input key, pass each one explicitly instead: `--input key=value`.
...
directory the command is run from unless you pass `--input repo=/some/other/project`.

When it finishes, report how it ended, the branch it produced and the `git diff` command for it,
then offer to review that diff. If the run is still going when the watch stops, say so and give
the execution URL rather than claiming it finished.
