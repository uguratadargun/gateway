...
export const CONSOLIDATOR_SYSTEM = `You maintain one team's page on one feature in an engineering team's memory. You are given the feature, the team's current summary of how it built it, and every decision the team's runs recorded under it — in the order they were made, each with its outcome and whether it still holds. The outcomes say how far the work got: "deployed" and "merged" are live, "shipped" is an older record of the same, "pr-open" means a merge request was opened and nobody has confirmed it landed, "completed" means the run finished but its pipeline never offered the work for merge, "unshipped" means the branch never got that far, "in-progress" means somebody taught a branch they said was unfinished and the choice may still move, and "abandoned" means the run failed or was stopped — which says nothing about the idea. A decision marked "refused" is different: the approach itself was turned down, by the reviewer, the verifier or a person, for the reason given. Do not write up a "pr-open" or "completed" decision as though it is live, say of an "in-progress" one that it is not settled yet, and never describe an abandoned decision as refused.

Write the page again, whole, from all of them:
- "summary": how this team's implementation works now, at the level of logic — the flow, the components and their responsibilities, the invariants, the edge cases handled. What a sibling team needs to build the same thing on another platform. Later decisions win over earlier ones where they conflict; refused and abandoned attempts are not the implementation, but a refusal's reason belongs in pitfalls, and what an unfinished attempt learned may too.
...
function describe(d: Decision): string {
  return truncate(
    [
      `### ${d.id} · ${d.outcome}${d.verdict === "rejected" ? ` · refused${d.verdictReason ? `: ${d.verdictReason}` : ""}` : ""} · from ${new Date(d.validFrom).toISOString().slice(0, 10)}${d.validTo ? ` to ${new Date(d.validTo).toISOString().slice(0, 10)} (no longer holds)` : ""}${d.supersedes ? ` · supersedes ${d.supersedes}` : ""}`,
      `title: ${d.title}`,
