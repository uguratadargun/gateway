    ...
    <main className="mx-auto max-w-4xl px-6 py-8">
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Local request/response log (truncated).</p>
        </div>
          ...
          {entries.map((e, i) => (
            <Card key={i} className="overflow-hidden">
              ...
              >
                ...
                <span className={`ml-auto text-xs ${e.status < 300 ? "text-emerald-500" : "text-destructive"}`}>
                  {e.status}
                </span>
              </button>
              {open === i && (
