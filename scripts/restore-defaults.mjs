...
/**
 * Runs src/scripts/restore-defaults.ts against this machine's ~/.gate.
 *
...
const dir = mkdtempSync(join(tmpdir(), "gate-restore-"));
const outfile = join(dir, "restore-defaults.mjs");
try {
  await build({
    entryPoints: [resolve(root, "src/scripts/restore-defaults.ts")],
    outfile,
    bundle: true,
    platform: "node",
    format: "esm",
